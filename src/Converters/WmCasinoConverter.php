 ...
 */
class WmCasinoConverter extends Converter
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->gameTypes = array_keys(config('api_caller.wm_casino.game_scopes'));
    }
    ...
    public function transform(array $rawTickets = [], string $userId = ''): array
    {
        ...
        foreach ($tickets as $ticket) {
            ...
            // =============================================
            $gameScope = array_get($ticket, 'gid', '');
            ...
            // 下注時間
            $betAt = array_get($ticket, 'betTime');
            // 結算時間
            $payoutAt = array_get($ticket, 'settime');

            // 若注單狀態為「Y」代表此局「有誤」需「作廢」
            $invalid = false;
            if(array_get($ticket,'reset') === 'Y') {
                $invalid = true;
            }

            switch (array_get($ticket, 'commission')) {
                case '0':
                    $commission = '一般';
                    break;
                case '1':
            ...
            if (array_get($ticket, 'gname') == '炸金花') {
                $column = str_replace("  ", "; ", array_get($ticket, 'gameResult', ''));
            } else {
                $column = str_replace(" ", '; ', array_get($ticket, 'gameResult', ''));
            }
            // 下注內容
            $gameResult = '<b style="color:#11bed1;">下注項目: '. array_get($ticket, 'betResult') . '</b><br/>';
            // 開牌結果
            $gameResult .= $column;
