 ...
 */
class ForeverEightConverter extends Converter
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->gameTypes = config('united_ticket.forever_eight.game_type');
        $this->ticketStatus = config('united_ticket.forever_eight.ticket_status');
    }
    ...
    public function transform(array $rawTickets = [], string $userId = ''): array
    {
        ...
        foreach ($tickets as $ticket) {
            ...
            // 顯示對應遊戲結果
            $ticketStatus = array_get($this->ticketStatus, array_get($ticket, 'TicketStatus'));

            // 計算輸贏結果
            $winningAmount = array_get($ticket, 'NetAmount') - array_get($ticket, 'BetValue');

            // 詳細
            $gameResult = '';
            $gameResult .= '<b style="color:#11bed1;">遊戲名稱: ' .  $game_type . ' </b><br/>';
//            $gameResult .= sprintf("細單編號: %s; ", array_get($ticket, 'BillNo', ''));
            $gameResult .= '<b style="color:#fc5a34;">遊戲结果: ' . $ticketStatus . ' </b>';

            $rawTicketArray = [
                // 原生注單的 uuid 識別碼等同於整合注單的識別碼 id
                'id' => array_get($ticket, 'uuid'),
                // 原生注單編號
                'bet_num' => (string)array_get($ticket, 'BillNo'),
                // 會員帳號識別碼
                ...
                'bet_type' => 'general',
                // 實際投注
                'raw_bet' => array_get($ticket, 'BetValue'),
                // 有效投注 (處理中洞、退組、合局情況之後的投注額)
                // 有效投注的資料一開始會和實際投注一樣，當開獎結果出來之後，才會做修正
                'valid_bet' => array_get($ticket, 'BetValue'),
                // 洗碼量 (扣掉同局對押情況之後的投注額)
                // 若沒有特別情況，該欄位資料應該會和實際投注一樣
                'rolling' => array_get($ticket, 'BetValue'),
                // 輸贏結果
                ...
                'invalid' => false,
                // 投注時間
                'bet_at' => array_get($ticket, 'SettleTime'),
                // 派彩時間
                'payout_at' => array_get($ticket, 'SettleTime'),
                // 資料建立時間
