...
class AutoFetchTicketCommand extends Command
{
    protected $console;
     ...
     * @var string
     */
    protected $signature = 'auto-fetch:ticket
        {station : 遊戲站識別碼}
        {user_id : 同步的目標ID}
        {username : 同步的目標帳號}
        {--password : 同步的目標密碼}';
    ...
    public function handle()
    {
        ...
        Log::channel('ticket-fetcher')->info(
            ...
            . '執行時間: ' . now()->toDateTimeString() . PHP_EOL
        );
    }
    ...
    private function curl($user)
    {
        ...
        // 實際API資料抓取，時間區間為自動設定，不需帶入
        $rawTickets = $fetcher->autoFetchTimeSpan()->capture();

        // 找出需轉換的注單
//        $enableTicketCompare = (env('ENABLE_TICKET_COMPARE', 'no') === 'yes');
//        $hasTickets = count($rawTickets['tickets']) > 0;
//        if($enableTicketCompare && $hasTickets) {
//            try {
//                $rawTickets = $fetcher->compare($rawTickets['tickets']);
        ...
        if (count($rawTickets['tickets']) > 0) {
            try {
                ...
                if ($this->station == 'super_lottery') {
                    $rawRakes = $fetcher->setRakeDate()->rakeCapture();
