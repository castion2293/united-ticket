 ...
 */
class WmCasinoFetcher extends Fetcher
{
    /**
     * 收集抓到的原始注單
     *
    ...
    private function curl()
    {
        ...
        // 取得注單資料並追加至原生注單收集器
        $rawTickets = array_get($arrData, 'result', []);

        if (filled($rawTickets)) {
            // 因為如果是單一一張注單，需把它加到一個陣列中的元素，避免錯誤
            $isOnlyOneTicket = array_has($rawTickets, 'betId');
            if ($isOnlyOneTicket) {
            ...
            foreach ($rawTickets as $rawTicket) {
                $rawTicketModel = new WmCasinoTicket($rawTicket);
    ...
    public function compare(array $fetchRawTickets): array
    {
        ...
        foreach ($uuidsChunk as $uuids) {
            $lastTicketsSet = DB::table('raw_tickets_wm_casino')
                ->select('uuid', 'reset')
                ->whereIn('uuid', $uuids)
                ->get()
                ->keyBy('uuid')
