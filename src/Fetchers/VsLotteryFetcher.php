 ...
 */
class VsLotteryFetcher extends Fetcher
{
    /**
     * 收集抓到的原始注單
     *
    ...
    public function setTimeSpan(string $fromTime = '', string $toTime = ''): Fetcher
    {
        ...
        if (empty($fromTime) && empty($toTime)) {
            $this->params['fromDate'] = Carbon::parse($fromTime)->toIso8601String();
            $this->params['toDate'] = Carbon::parse($toTime)->toIso8601String();
            return $this;
        }
    ...
    {
        $this->params['fromDate'] = now()->subDays(1)->toIso8601String();
        $this->params['toDate'] = now()->toIso8601String();
    ...
    public function  capture(): array
    {
        try {
            ...
            $this->consoleOutput->writeln(join(PHP_EOL, [
                ...
                "-------------------------------------",
                "　　遊戲站: vs_lottery                  ",
                "　開始時間: {$this->params['fromDate']}",
                "　結束時間: {$this->params['toDate']}",
                "--",
                ""
            ]));
            ...
            $this->consoleOutput->writeln(sprintf("收集共 %d 筆，已無注單，停止程序" . PHP_EOL,
    ...
    private function curl()
    {
        ...
        // 取得注單資料並追加至原生注單收集器
        $rawTickets = array_get($arrData, 'trans', []);
        $rawTickets = array_get($rawTickets, 'Trans.TransactionDetail', []);

        if (filled($rawTickets)) {
            // 因為如果是單一一張注單，需把它加到一個陣列中的元素，避免錯誤
            $isOnlyOneTicket = array_has($rawTickets, 'TrDetailID');
            if ($isOnlyOneTicket) {
            ...
            foreach ($rawTickets as $rawTicket) {
                ...
                $rawTicketModel = new VsLotteryTicket($rawTicket);
                ...
                // BetNo 有可能是空的
                $ticketSet['BetNo'] = json_encode(array_get($rawTicket, 'BetNo'));
        ...
        if(count($rawTickets) == 100) {
            $this->consoleOutput->writeln(sprintf("收集共 %d 筆，尚有注單" . PHP_EOL,
                    count($rawTickets))
            );
            $this->params['fromRowNo'] = array_get(end($rawTickets), 'FetchId') + 1;
            $this->curl();
        }
    ...
    public function compare(array $fetchRawTickets): array
    {
        ...
        foreach ($uuidsChunk as $uuids) {
            $lastTicketsSet = DB::table('raw_tickets_vs_lottery')
                ->select('uuid', 'WinLossStatus')
                ->whereIn('uuid', $uuids)
                ->get()
                ->keyBy('uuid')
                ->toArray();
        ...
        $tickets = collect($fetchRawTickets)->filter(function ($fetchRawTicket) use ($lastTickets) {
            ...
            $fetchRawTicketState = array_get($fetchRawTicket, 'WinLossStatus');
            $lastTicketState = data_get($lastTicket, 'WinLossStatus');
