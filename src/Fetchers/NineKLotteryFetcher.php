...
class NineKLotteryFetcher extends Fetcher
{
    /**
     * 收集抓到的原始注單
     *
    ...
    public function __construct(array $user)
    {
        ...
        $this->agentId = config('api_caller.nine_k_lottery.config.agent_id');
    ...
    private function curl()
    {
        ...
        if (filled($tickets)) {
            ...
            foreach ($tickets as $ticketSet) {
                $rawTicketModel = new NineKLotteryTicket($ticketSet);
    ...
    public function compare(array $fetchRawTickets): array
    {
        ...
        foreach ($uuidsChunk as $uuids) {
            $lastTicketsSet = DB::table('raw_tickets_nine_k_lottery')
                ->select('uuid', 'Result')
                ->whereIn('uuid', $uuids)
                ->get()
                ->keyBy('uuid')
